...
| Schema migration | Manifest, task, server/client layout, config, lock, and sync-state formats have independent v1 markers. Future versions fail closed. Published manifests are never silently rewritten, SQLite is preserved and rebuilt, task/lock bootstrap migrations are atomic, and a future lock cannot be silently regenerated by an older CLI. |
