...
| Client resumable staging state       | Interrupted local sync work                 | `models/.staging/*/.modelshelf/staging.json`            | Reuse only when artifact ID and content digest match; otherwise retain the old tree under `.retained-*/data` before copying. |
